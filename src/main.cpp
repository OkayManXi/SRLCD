...
int32_t main() {
	...
	for(int num_count =0; num_count< video_size; num_count++){
		...
		if(num_count>=1000){
			imagefile = dataset+std::to_string(num_count)+".png";
		}
		else if(num_count>=100){
			imagefile = dataset+"0"+std::to_string(num_count)+".png";
		}
		else if(num_count>=10){
			imagefile = dataset+"00"+std::to_string(num_count)+".png";
		}
		else 
		...
		for (int p = 0;p < sr_arr.size();p++) {
			...
			for (int i = 0; i < loop_candidate.size(); i++)
			{
				if (temporal_reidentification(loop_result, num_count)) {
					...
					if (geometry_reidentification_feature_matching(videos, num_count, loop_candidate[i].loop_frame)) {
						loop_result(num_count, 0) = 1;
						//std::cout << "loop detecterd" << std::endl;
						//fout << "t1=" << loop_candidate[i].current_frame << '\t' << "coincides with t2=" << loop_candidate[i].loop_frame << "\n";
						break;
...
bool geometry_reidentification_feature_matching(std::vector<cv::Mat>& videos, int i, int j) {
	cv::Mat image1 = videos[i];
	cv::Mat image2 = videos[j];
...
void groundtruth_test(void) {
	...
	for(int num_count =0; num_count< video_size; num_count++){
		...
		std::string imagefile;
		if(num_count>=1000){
			imagefile = dataset+std::to_string(num_count);
		}
		else if(num_count>=100){
			imagefile = dataset+"0"+std::to_string(num_count);
		}
		else if(num_count>=10){
			imagefile = dataset+"00"+std::to_string(num_count);
		}
		else 
			imagefile = dataset+"000"+std::to_string(num_count);
	...
	for (int i = 0; i < video_size; i++)
	{
		...
		{
			if (geometry_reidentification_feature_matching(videos, i, j)) {
